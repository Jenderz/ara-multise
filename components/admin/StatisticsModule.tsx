            ...
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 bg-white dark:bg-zinc-900 p-4 rounded-3xl border border-gray-100 dark:border-white/5 shadow-sm">
                <div>
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mt-1.5">
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1 shrink-0 ${isGlobalView ? 'bg-purple-100 text-purple-700' : 'bg-orange-100 text-orange-700'}`}>
                ...
                <div className="bg-gray-100 dark:bg-white/10 p-1.5 rounded-2xl grid grid-cols-3 xl:flex gap-1 w-full xl:w-auto">
                    <button onClick={() => setViewMode('financial')} className={`px-2 sm:px-5 py-2 sm:py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 sm:gap-2 ${viewMode === 'financial' ? 'bg-white dark:bg-zinc-800 text-ios-blue shadow-sm' : 'text-gray-500 hover:text-gray-800 dark:hover:text-white'}`}>
                        <DollarSign size={16} className="shrink-0"/> <span className="truncate">Ventas</span>
                    </button>
                    <button onClick={() => setViewMode('inventory')} className={`px-2 sm:px-5 py-2 sm:py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 sm:gap-2 ${viewMode === 'inventory' ? 'bg-white dark:bg-zinc-800 text-purple-600 dark:text-purple-400 shadow-sm' : 'text-gray-500 hover:text-gray-800 dark:hover:text-white'}`}>
                        <Package size={16} className="shrink-0"/> <span className="truncate">Stock</span>
                    </button>
                    <button onClick={() => setViewMode('sellers')} className={`px-2 sm:px-5 py-2 sm:py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 sm:gap-2 ${viewMode === 'sellers' ? 'bg-white dark:bg-zinc-800 text-yellow-600 dark:text-yellow-400 shadow-sm' : 'text-gray-500 hover:text-gray-800 dark:hover:text-white'}`}>
                        <Trophy size={16} className="shrink-0"/> <span className="truncate">Vendedores</span>
                    </button>
                </div>
            </div>
